  ...
  <div class="questionbank">
    ...
    <div class="header">
      <div class="filter-controls">
        <a-input-search v-model:value="searchQuery" placeholder="搜索题库名称" class="input-search" allow-clear :button="true"
          @search="handleSearch" />
        <a-select v-model:value="bankStatus" placeholder="状态" class="select-filter" @change="handleStatusChange">
          ...
          <div v-if="!loading && filteredBankList.length > 0" class="bank-list">
            <div v-for="bank in filteredBankList" :key="bank.id" class="bank-card">
              <div class="bank-header">
                <div class="icon-box">
                  <img v-if="bank.picture" :src="bank.picture" :alt="bank.title" class="bank-icon-display uploaded-icon"
                ...
                <div class="bank-actions">
                  <button class="action-container" type="button" @click="showEditModal(bank)" title="编辑题库">
              ...
              <div class="bank-stats">
                <div class="progress">
                  ...
                  <span class="completion-rate-value">
                    {{ bank.completionRate || 0 }}%
                  </span>
                </div>
    ...
    <!-- 新增题库 -->
    <a-modal v-model:visible="addModalVisible" title="新增题库" width="600px" @ok="handleAddBank" @cancel="handleCancelAdd"
      ...
      <a-form :model="addForm" :rules="bankFormRules" ref="addFormRef" :label-col="{ span: 4 }"
        ...
        <a-form-item label="题库图标" name="picture">
          <a-upload v-model:file-list="addForm.fileList" :before-upload="beforeUpload" :max-count="1" accept="image/*"
    ...
    <a-modal v-model:visible="editModalVisible" title="编辑题库" width="600px" @ok="handleEditBank"
      ...
      <a-form :model="editForm" :rules="bankFormRules" ref="editFormRef" :label-col="{ span: 4 }"
        ...
        <a-form-item label="题库图标" name="picture">
          <a-upload v-model:file-list="editForm.fileList" :before-upload="beforeUpload" :max-count="1" accept="image/*"
            list-type="picture-card" :show-upload-list="true">
        ...
        <a-form-item label="统计信息">
          <div class="statistics-info">
            ...
            <p>
              状态:
              <span :class="getStatusClass(editForm.isDelete)">{{ getStatusText(editForm.isDelete) }}</span>
            </p>
...
import { getQuestionCountByBankId } from '@/apis/questionBankQuestionApi';
...
const bankStatus = ref<string>('all');
...
// 经过前端过滤后的题库列表
const filteredBankList = computed<QuestionBank[]>(() => {
  ...
  if (bankStatus.value !== 'all') {
    const isDeleteValue = parseInt(bankStatus.value, 10);
...
// 空状态的描述文字
const getEmptyDescription = computed<string>(() => {
...
// 难度样式（这里主要是用来显示合并后的 avgDifficulty）
...
// 合并题库基本信息和统计信息，然后再获取“题目数量”
const mergeBankData = async (
  basicBanks: QuestionBank[],
  analyzeBanks: QuestionBankAnalyze[]
): Promise<QuestionBank[]> => {
  // 把 analyzeBanks 组织成 Map：key=ID, value=对应的统计对象
  const analyzeMap = new Map<string, QuestionBankAnalyze>();
  analyzeBanks.forEach(analyze => {
    analyzeMap.set(analyze.id, analyze);
  });

  // 第一步：先合并“基本信息”和“后端统计”到一个数组
  const merged: QuestionBank[] = basicBanks.map(bank => {
    const analyze = analyzeMap.get(bank.id);
    return {
      ...bank,
      questionCount: analyze?.questionCount || 0, // 这个数值后面会被覆盖
      completionRate: analyze?.completeRate
        ? parseFloat(analyze.completeRate.replace('%', ''))
        : 0,
      avgDifficulty: analyze?.averageDifficulty || '未知',
      activeLevel: analyze?.activity || '低',
      updateTime: bank.updateTime,
      isDelete: bank.isDelete
    };
  });

  // 第二步：真正“按题库去拉取题目数量”，覆盖 questionCount
  await Promise.all(
    merged.map(async bank => {
      try {
        const resp = await getQuestionCountByBankId(bank.id);
        if (resp.code === 0) {
          bank.questionCount = resp.data as number;
        } else {
          console.warn(`获取题库[${bank.id}]的题目数量失败:`, resp.message);
        }
      } catch (e) {
        console.error(`获取题库[${bank.id}]的题目数量 出错:`, e);
      }
    })
  );
...
const getQuestionBanks = async () => {
  try {
    ...
    if (basicResponse.code === 0 && analyzeResponse.code === 0) {
      // 把后端返回的 basicResponse.data 转成 QuestionBank[]
      const basicBanks: QuestionBank[] = (basicResponse.data || []).map((item: any) => ({
        id: String(item.id),
        title: item.title,
      ...
      // 把后端返回的 analyzeResponse.data 转成 QuestionBankAnalyze[]
      const analyzeBanks: QuestionBankAnalyze[] = (analyzeResponse.data || []).map((item: any) => ({
        id: String(item.id),
        title: item.title,
      ...
      // 合并基本信息和统计信息，并且再去拉取每个题库真正的 “题目数量”
      const merged = await mergeBankData(basicBanks, analyzeBanks);
      bankList.value = merged;
      ...
      message.destroy('loadingMessage');
      const errorMsg =
        basicResponse.code !== 0 ? basicResponse.message : analyzeResponse.message;
